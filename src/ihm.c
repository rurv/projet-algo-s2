...
enum EcranActuel menu_principale(enum EcranActuel ecran, BITMAP *buffer) {
    ...
    ecrire_centre_texte(buffer, "Quitter", SCREEN_WIDTH/2, LY(0.750), makecol(200, 200, 200), 4);
...
enum EcranActuel choix(enum EcranActuel ecran, BITMAP *buffer, Assets *assets, Player *p) {
    ...
    if (mouse_b & 1) {
        if (mouse_x > LX(0.2) && mouse_x < LX(0.2)+LX(VAISSEAU_L) &&
            ...
            p->skin_id = VAISSEAU1; ATTENDRE_RELACHE(); return DECOMPTE;
        } else if (mouse_x > LX(0.4) && mouse_x < LX(0.4)+LX(VAISSEAU_L) &&
                   mouse_y > LY(0.5) && mouse_y < LY(0.5)+LY(VAISSEAU_H)) {
            p->skin_id = VAISSEAU2; ATTENDRE_RELACHE(); return DECOMPTE;
        } else if (mouse_x > LX(0.6) && mouse_x < LX(0.6)+LX(VAISSEAU_L) &&
                   mouse_y > LY(0.5) && mouse_y < LY(0.5)+LY(VAISSEAU_H)) {
            p->skin_id = VAISSEAU3; ATTENDRE_RELACHE(); return DECOMPTE;
        } else if (mouse_x > LX(0.8) && mouse_x < LX(0.8)+LX(VAISSEAU_L) &&
                   mouse_y > LY(0.5) && mouse_y < LY(0.5)+LY(VAISSEAU_H)) {
            p->skin_id = VAISSEAU4; ATTENDRE_RELACHE(); return DECOMPTE;
        }
    }
    return ecran;
}
...
enum EcranActuel decompte(enum EcranActuel ecran, BITMAP *buffer, Assets *assets, Player *p) {
    ...
    if (etat_decompte == DECOMPTE1) {
        ...
        int v_w = LX(VAISSEAU_L), v_h = LY(VAISSEAU_H);
        masked_stretch_blit(assets->player_sprites, buffer,
        ...
        if (mouse_b & 1) { ATTENDRE_RELACHE(); return JEU; }
...
enum EcranActuel pause_level(enum EcranActuel ecran, BITMAP *buffer) {
    ...
    if (mouse_b & 1) {
        if (mouse_x > LX(0.3125) && mouse_x < LX(0.6875) &&
            mouse_y > LY(0.333)  && mouse_y < LY(0.433)) {
            ATTENDRE_RELACHE(); return JEU;
        } else if (mouse_x > LX(0.3125) && mouse_x < LX(0.6875) &&
                   mouse_y > LY(0.467)  && mouse_y < LY(0.567)) {
            ATTENDRE_RELACHE(); return SAUVEGARDE;
        } else if (mouse_x > LX(0.3125) && mouse_x < LX(0.6875) &&
                   mouse_y > LY(0.600)  && mouse_y < LY(0.700)) {
            ATTENDRE_RELACHE(); return REGLE;
        } else if (mouse_x > LX(0.3125) && mouse_x < LX(0.6875) &&
                   mouse_y > LY(0.733)  && mouse_y < LY(0.800)) {
            ATTENDRE_RELACHE(); return MENU_PRINCIPAL;
        }
    }
    return ecran;
}

// Écran de game over avec dialogue Claude avant les boutons
enum EcranActuel game_over_screen(enum EcranActuel ecran, BITMAP *buffer, Player *p,
    ...
    // Dialogue Claude en priorité : bloque les boutons tant qu'il n'est pas terminé
    if (!dialogue->done) {
    ...
    // Boutons accessibles uniquement après le dialogue
...
enum EcranActuel victory_screen(enum EcranActuel ecran, BITMAP *buffer, Player *p) {
    // Titre
    ecrire_centre_texte(buffer, "VICTOIRE !", SCREEN_WIDTH/2, LY(0.12), makecol(255, 215, 0), 10);

    char msg[50];
    snprintf(msg, sizeof(msg), "Felicitations, %s !", p->pseudo);
    ecrire_centre_texte(buffer, msg, SCREEN_WIDTH/2, LY(0.32), makecol(255, 255, 255), 3);
    ecrire_centre_texte(buffer, "La Terre est sauvee grace a vous.", SCREEN_WIDTH/2, LY(0.42), makecol(200, 200, 200), 2);

    // Bouton recommencer
    rectfill(buffer, LX(0.3125), LY(0.52), LX(0.6875), LY(0.60), makecol(40, 80, 40));
    ecrire_centre_texte(buffer, "Recommencer",   SCREEN_WIDTH/2, LY(0.54), makecol(255, 255, 255), 4);

    // Bouton menu principal
    rectfill(buffer, LX(0.3125), LY(0.64), LX(0.6875), LY(0.72), makecol(30, 30, 50));
    ecrire_centre_texte(buffer, "Menu principal",SCREEN_WIDTH/2, LY(0.66), makecol(200, 200, 200), 4);
    ...
    if (mouse_b & 1) {
        if (mouse_x > LX(0.3125) && mouse_x < LX(0.6875) &&
            mouse_y > LY(0.52)   && mouse_y < LY(0.60)) {
            ATTENDRE_RELACHE(); return CHOIX;
        } else if (mouse_x > LX(0.3125) && mouse_x < LX(0.6875) &&
                   mouse_y > LY(0.64)   && mouse_y < LY(0.72)) {
            ATTENDRE_RELACHE(); return MENU_PRINCIPAL;
        } else if (mouse_x > LX(0.3125) && mouse_x < LX(0.6875) &&
                   mouse_y > LY(0.76)   && mouse_y < LY(0.81)) {
            ATTENDRE_RELACHE(); return QUITTER;
