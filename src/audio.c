...
void audio_init(Audio *a) {
    ...
    if (!a->hit_sfx) { allegro_message("Erreur chargement hit.wav"); allegro_exit(); exit(1); }
    ...
    if (!a->boss_eclair) { allegro_message("Erreur chargement boss_eclair.wav"); allegro_exit(); exit(1); }

    a->bo_sfx = load_sample("assets/sfx/bo.wav");
    if (!a->bo_sfx) a->bo_sfx = load_sample("../assets/sfx/bo.wav");
    if (!a->bo_sfx) { allegro_message("Erreur chargement bo.wav"); allegro_exit(); exit(1); }
}
